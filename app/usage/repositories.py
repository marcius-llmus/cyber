...
class UsageRepository(BaseRepository[SessionUsage]):
    ...
    async def increment_usage(
        self, session_id: int, cost: Decimal, input_tokens: int, output_tokens: int, cached_tokens: int
    ) -> SessionUsage:
...
class GlobalUsageRepository(BaseRepository[GlobalProviderUsage]):
    ...
    async def increment_provider_usage(
        self, provider: str, cost: Decimal, input_tokens: int, output_tokens: int, cached_tokens: int
    ) -> GlobalProviderUsage:
        ...
        stmt = insert(self.model).values(
            ...
            set_={
                ...
                "total_output_tokens": self.model.total_output_tokens + output_tokens,
                "total_cached_tokens": self.model.total_cached_tokens + cached_tokens,
            }
        ).returning(self.model)
