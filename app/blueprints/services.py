...
class BlueprintService:
    ...
    async def get_blueprint_content(self, path: str) -> str:
        try:
            ...
            files = await self.codebase_service.resolve_file_patterns(blueprint_root_str, ["."])
            read_results = await self.codebase_service.read_files(blueprint_root_str, files)
        except Exception as e:
            logger.error(f"Error reading blueprint content for path '{path}': {e}", exc_info=True)
            raise e

        content_blocks = []
        for result in read_results:
            if result.status == FileStatus.SUCCESS:
                content_blocks.append(f'<file path="{result.file_path}">\n{result.content}\n</file>')
