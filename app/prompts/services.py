...
class PromptService:
    ...
    async def create_global_prompt(self, prompt_in: PromptCreate) -> Prompt:
        ...
        internal_create = PromptInternalCreate(
            type=PromptType.GLOBAL, project_id=None, **prompt_in.model_dump()
        )
    ...
    async def create_project_prompt(self, prompt_in: PromptCreate) -> Prompt:
        ...
        if not active_project:
            raise ActiveProjectRequiredException(
                "An active project is required to create a project prompt."
            )
        internal_create = PromptInternalCreate(
            type=PromptType.PROJECT,
    ...
    async def delete_prompt(self, prompt_id: int) -> None:
        await self.get_prompt(
            prompt_id
        )  # Ensures the prompt exists before attempting deletion
    ...
    async def get_active_prompts(self, project_id: int) -> list[Prompt]:
        ...
        if blueprint_prompt := await self.prompt_repo.find_project_blueprint_prompt(
            project_id
        ):
    ...
    async def create_or_update_project_blueprint_prompt(
        self, path: str
    ) -> tuple[Prompt, bool]:
        """
        ...
        if not active_project:
            raise ActiveProjectRequiredException(
                "An active project is required to create a blueprint prompt."
            )

        existing_prompt = await self.prompt_repo.find_project_blueprint_prompt(
            project_id=active_project.id
        )
        ...
        if existing_prompt:
            ...
            updated_prompt = await self.update_prompt(
                prompt_id=existing_prompt.id, prompt_in=update_data
            )
    ...
    async def get_project_blueprint_prompt(self) -> Prompt | None:
        ...
        if not active_project:
            return None
        return await self.prompt_repo.find_project_blueprint_prompt(
            project_id=active_project.id
        )
    ...
    async def delete_project_blueprint_prompt(self) -> None:
        ...
        if not active_project:
            raise ActiveProjectRequiredException(
                "An active project is required to delete a blueprint prompt."
            )
        ...
        if existing_prompt:
            ...
            raise PromptNotFoundException(
                "Blueprint prompt not found for the active project."
            )

    async def toggle_active_attachment_for_current_project(
        self, prompt_id: int
    ) -> tuple[Prompt, bool]:
        ...
        if not active_project:
            raise ActiveProjectRequiredException(
                "An active project is required to attach or detach prompts."
            )

        prompt = await self.get_prompt(prompt_id)

        attachment = await self.prompt_repo.find_attachment(
            prompt_id=prompt_id, project_id=active_project.id
        )

        if attachment:
            await self.prompt_repo.detach_from_project(attachment)
        else:
            await self.prompt_repo.attach_to_project(
                prompt_id=prompt_id, project_id=active_project.id
            )
...
class PromptPageService:
    ...
    async def _get_attached_prompt_ids(
        ...
        return {
            ...
            for attachment in await self.prompt_service.prompt_repo.get_project_attachments(
                project_id=active_project.id
            )
        }
    ...
    @staticmethod
    def _truncate_prompt_content(
        prompt: Prompt, truncate_length: int = 200
    ) -> PromptRead:
    ...
    async def get_blueprint_prompts_list_data(self) -> dict:
        ...
        if current_prompt := await self.prompt_service.get_project_blueprint_prompt():
            current_prompt = self._truncate_prompt_content(
                current_prompt, truncate_length=3000
            )
