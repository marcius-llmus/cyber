...
class ProjectPromptAttachment(Base):
    ...
    prompt_id = Column(
        Integer, ForeignKey("prompts.id", ondelete="CASCADE"), primary_key=True
    )

    project = relationship(
        "Project", back_populates="prompt_attachments", lazy="joined"
    )
...
class SessionPromptAttachment(Base):
    ...
    session = relationship(
        "ChatSession", back_populates="prompt_attachments", lazy="joined"
    )
