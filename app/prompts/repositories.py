...
class PromptRepository(BaseRepository[Prompt]):
    ...
    async def list_global(self) -> list[Prompt]:
        stmt = (
            select(self.model)
            .where(self.model.type == PromptType.GLOBAL)
    ...
    async def list_by_project(self, project_id: int) -> list[Prompt]:
        stmt = (
            ...
            .where(
                ...
                self.model.type == PromptType.PROJECT,
            )
            .order_by(self.model.name)
        )
        result = await self.db.execute(stmt)
    ...
    async def find_attachment(
        self, prompt_id: int, project_id: int
    ) -> ProjectPromptAttachment | None:
        stmt = select(ProjectPromptAttachment).filter_by(
            prompt_id=prompt_id, project_id=project_id
        )
        result = await self.db.execute(stmt)
        return result.scalars().first()
    ...
    async def get_attached_prompts(self, project_id: int) -> list[Prompt]:
        stmt = (
            select(Prompt)
            .join(ProjectPromptAttachment)
            .where(ProjectPromptAttachment.project_id == project_id)
        )
        result = await self.db.execute(stmt)
        return list(result.scalars().all())
