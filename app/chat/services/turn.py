...
class ChatTurnService:
    ...
    async def start_turn(self, *, session_id: int, turn_id: str | None = None) -> str:
        if turn_id is None:
            ...
            await self.turn_repo.create(
                obj_in=ChatTurnCreate(id=new_turn_id, session_id=session_id, status=ChatTurnStatus.PENDING)
            )
            return new_turn_id

        existing = await self.turn_repo.get_by_id_and_session(turn_id=turn_id, session_id=session_id)
        if not existing:
            raise ValueError(f"Retry requested for turn {turn_id}, but it does not exist")
    ...
    async def mark_succeeded(self, *, session_id: int, turn_id: str) -> None:
        turn = await self.turn_repo.get_by_id_and_session(turn_id=turn_id, session_id=session_id)
        if not turn:
