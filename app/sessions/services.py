...
class SessionService:
    def __init__(
        self, session_repo: ChatSessionRepository, project_service: ProjectService
    ):
    ...
    async def get_most_recent_session_by_project(
        self, project_id: int
    ) -> ChatSession | None:
    ...
    async def set_active_session(self, session_id: int) -> ChatSession:
        ...
        await self.session_repo.deactivate_all_for_project(
    ...
    async def set_operational_mode(
        self, session_id: int, mode: OperationalMode
    ) -> ChatSession:
        return await self._update_session(
            session_id=session_id, obj_in=ChatSessionUpdate(operational_mode=mode)
        )
    ...
    async def rename_session(self, session_id: int, name: str) -> ChatSession:
        ...
        return await self._update_session(
            session_id=session_id, obj_in=ChatSessionUpdate(name=normalized)
        )

    async def _update_session(
        self, session_id: int, obj_in: ChatSessionUpdate
    ) -> ChatSession:
        session = await self.get_session(session_id=session_id)
...
class SessionPageService:
    def __init__(
        self, session_service: SessionService, project_service: ProjectService
    ):
    ...
    async def get_sessions_page_data(self) -> dict:
        active_project = await self.project_service.get_active_project()
        sessions = (
            await self.session_service.get_sessions_by_project(
