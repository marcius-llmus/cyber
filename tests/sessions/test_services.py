...
async def test_set_active_session_orchestration(
    ...
    chat_session_repository_mock.get_with_messages.assert_awaited_once_with(
        session_id=session_id
    )
    chat_session_repository_mock.deactivate_all_for_project.assert_awaited_once_with(
        project_id=456
    )
    chat_session_repository_mock.activate.assert_awaited_once_with(mock_session)

    assert project_service_mock.set_active_project.call_args_list == [
        call(456),
        call(project_id=456),
    ]
...
async def test_set_active_session_raises_when_session_not_found(
    ...
    with pytest.raises(
        ChatSessionNotFoundException, match="Session with id 999 not found"
    ):
        await session_service.set_active_session(999)


async def test_delete_session_checks_active_project(
    session_service: SessionService, project_service_mock: MagicMock
):
...
async def test_delete_session_returns_active_status(
    session_service: SessionService,
    chat_session_repository_mock: MagicMock,
    project_service_mock: MagicMock,
):
...
async def test_rename_session(
    session_service: SessionService, chat_session_repository_mock: MagicMock
):
...
async def test_get_operational_mode(
    session_service: SessionService, chat_session_repository_mock: MagicMock
):
    """Verify getter."""
...
async def test_set_operational_mode(
    session_service: SessionService, chat_session_repository_mock: MagicMock
):
    """Verify setter."""
    mock_session = MagicMock()
...
async def test_get_most_recent_session_by_project(
    session_service: SessionService, chat_session_repository_mock: MagicMock
):
    """Verify delegates to repo."""
    await session_service.get_most_recent_session_by_project(1)
    chat_session_repository_mock.get_most_recent_by_project.assert_awaited_once_with(
        project_id=1
    )
