...
async def test_set_active_session_orchestration(session_service: SessionService, chat_session_repository_mock: MagicMock, project_service_mock: MagicMock):
    ...
    # Execute
    await session_service.set_active_session(session_id)

    # Verify
    assert project_service_mock.set_active_project.call_count == 2
    project_service_mock.set_active_project.assert_any_call(456)
    chat_session_repository_mock.deactivate_all_for_project.assert_called_once_with(project_id=456)
    chat_session_repository_mock.activate.assert_called_once_with(mock_session)
...
async def test_delete_session_checks_active_project(session_service: SessionService, project_service_mock: MagicMock):
    ...
    project_repo_mock.get_active.return_value = None
...
async def test_delete_session_returns_active_status(session_service: SessionService, chat_session_repository_mock: MagicMock, project_service_mock: MagicMock):
    ...
    # Setup
    project_repo_mock = AsyncMock()
    project_repo_mock.get_active.return_value = MagicMock(id=1)
    project_service_mock.project_repo = project_repo_mock
    ...
    chat_session_repository_mock.delete.assert_called_with(pk=123)
    ...
    chat_session_repository_mock.delete.assert_called_with(pk=456)
...
async def test_rename_session(session_service: SessionService, chat_session_repository_mock: MagicMock):
    ...
    chat_session_repository_mock.update.assert_called_once()
...
async def test_session_page_service_logic(session_service_mock: MagicMock, project_service_mock: MagicMock):
    ...
    project_repo_mock.get_active.return_value = mock_project
    session_service_mock.get_sessions_by_project.return_value = ["s1", "s2"]

    data = await page_service.get_sessions_page_data()
    assert data["sessions"] == ["s1", "s2"]
    session_service_mock.get_sessions_by_project.assert_called_once_with(project_id=99)
...
async def test_create_session(session_service: SessionService, chat_session_repository_mock: MagicMock):
    ...
    chat_session_repository_mock.create.assert_called_once_with(obj_in=session_in)
...
async def test_get_sessions_by_project(session_service: SessionService, chat_session_repository_mock: MagicMock):
    ...
    chat_session_repository_mock.list_by_project.assert_called_once_with(project_id=1)
...
async def test_get_most_recent_session_by_project(session_service: SessionService, chat_session_repository_mock: MagicMock):
    ...
    chat_session_repository_mock.get_most_recent_by_project.assert_called_once_with(project_id=1)
