...
class TestMessageRepository:
    async def test_list_by_session_id_returns_empty_list_when_no_messages(self, message_repository, chat_session):
    ...
    async def test_list_by_session_id_orders_by_id_ascending(self, message_repository, chat_session, db_session):
...
class TestChatTurnRepository:
    async def test_get_by_id_and_session_returns_none_when_missing(self, chat_turn_repository, chat_session):
        """get_by_id_and_session returns None when no row matches turn_id+session_id."""
        turn = await chat_turn_repository.get_by_id_and_session(turn_id="missing", session_id=chat_session.id)
        assert turn is None

    async def test_get_by_id_and_session_returns_row_when_present(self, chat_turn_repository, chat_session, db_session):
        """get_by_id_and_session returns a ChatTurn when it exists for the session."""
        turn = ChatTurn(id="t1", session_id=chat_session.id, status=ChatTurnStatus.PENDING)
        db_session.add(turn)
        await db_session.flush()

        result = await chat_turn_repository.get_by_id_and_session(turn_id="t1", session_id=chat_session.id)
