...
class TestChatDependencies:
    async def test_get_message_repository_returns_repository_instance(self, db_session):
        """get_message_repository returns a MessageRepository bound to the request db session."""
        repo = await get_message_repository(db=db_session)
    ...
    async def test_get_chat_service_wires_message_repo_session_service_project_service(self, db_session):
        """get_chat_service returns ChatService wired with MessageRepository + SessionService + ProjectService."""
        service = await get_chat_service(db=db_session)
    ...
    async def test_get_chat_turn_service_wires_chat_turn_repository(self, db_session):
        """get_chat_turn_service returns ChatTurnService wired with ChatTurnRepository."""
        service = await get_chat_turn_service(db=db_session)
        assert isinstance(service, ChatTurnService)
        assert isinstance(service.turn_repo, ChatTurnRepository)

    async def test_get_chat_service_propagates_factory_errors(self, db_session):
        ...
        with patch("app.chat.dependencies.build_chat_service", side_effect=ValueError("Factory error")):
            with pytest.raises(ValueError, match="Factory error"):
                await get_chat_service(db=db_session)

    async def test_get_chat_turn_service_propagates_factory_errors(self, db_session):
        ...
        with patch("app.chat.dependencies.build_chat_turn_service", side_effect=ValueError("Factory error")):
            with pytest.raises(ValueError, match="Factory error"):
                await get_chat_turn_service(db=db_session)
